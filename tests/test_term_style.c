...
static double check_palette(const uint32_t source[10]) {
    ...
    for (int i=0; i<TERM_FACE_COUNT; ++i) {
        ...
        for (int m=TERM_COLORS_AUTO; m<=TERM_COLORS_MONO; ++m) {
            ...
            CHECK((strstr(text,"38;2;") != NULL) == (m==TERM_COLORS_AUTO || m==TERM_COLORS_RGB));
            CHECK((strstr(text,"38;5;") != NULL) == (m==TERM_COLORS_AUTO || m==TERM_COLORS_256));
            if (m==TERM_COLORS_AUTO) CHECK(strstr(text,"38;5;") < strstr(text,"38;2;"));
            free(text);
        }
    }
...
int main(void) {
    ...
    CHECK(term_colors_resolve(TERM_COLORS_AUTO,"xterm",NULL)==TERM_COLORS_AUTO);
    CHECK(term_colors_resolve(TERM_COLORS_AUTO,"putty",NULL)==TERM_COLORS_AUTO);
    CHECK(term_colors_resolve(TERM_COLORS_AUTO,"screen-256color",NULL)==TERM_COLORS_AUTO);
    CHECK(term_colors_resolve(TERM_COLORS_AUTO,"tmux-256color",NULL)==TERM_COLORS_AUTO);
    ...
    printf("PASS: 257 palettes; 7:1 RGB and indexed contrast; no ANSI slots 0-15; cached palette; colour modes\n");
